...
interface NameInputQuestionProps {
  question: Question
}

export default function NameInputQuestion({ question }: NameInputQuestionProps) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newValue.trim()) {
      ...
      // Also store as participant_name for easier access
      handleAnswer("participant_name", newValue.trim())

      console.log(`Name input question answered: ${question.fieldName} = ${newValue.trim()}`)
    }
  }
    ...
    >
      <div className="w-full max-w-md">
        ...
        {question.required && value.trim() === "" && (
          <p className="text-red-500 mt-2 text-sm">Your name is required to proceed</p>
        )}

        <p className="text-gray-500 mt-4 text-center">We'll include your name with your quiz results</p>
      </div>
    </motion.div>
  )
}
